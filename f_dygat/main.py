...
from DyGCN.intrusion_detection import OCSVM
import pandas as pd
def parse():
    parser=argparse.ArgumentParser()
    parser.add_argument('--mode',default= 'train', type=str) # 具体进行训练还是测试
    parser.add_argument('--ck_path',default='DyGCN/savedmodel/cst2/model.pt', type=str) # 模型保存路径
    parser.add_argument('--dataset', default='cst2', type=str)
    parser.add_argument('--learning_rate', default=0.0001,type=float)
    parser.add_argument('--in_dim', default=69,type=int) # 边属性特征初始维度
...
if __name__ =='__main__':
    ...
    print('开始图嵌入特征提取 .....')   
    data_embs, flow_with_preds = None, None
    data = None
    ...
    if args.mode=='train':
        ...
        t0=time()   
        print('model test .....')
        data_embs, flow_with_preds = predict5(args, data)
        ...
        data_embs, flow_with_preds = predict5(args, data)
        print('DyGCN 测试时间：', time()-t0)
    elif args.mode=='intrusion_dection':
        graph_embs_path = os.path.join('DyGCN/data/', args.dataset, 'graph_embs.pt')
        flow_with_preds_path = os.path.join('DyGCN/data/', args.dataset, 'flow_with_preds.pt')

        if os.path.exists(graph_embs_path):
            data_embs = torch.load(graph_embs_path)
            flow_with_preds = pd.read_csv(flow_with_preds_path).values.reshape(-1,1000,5)[args.seq_len:]

    print('开始异常检测 .....')           
    train_data = data_embs[train_len[0]+args.seq_len:train_len[1]]
    print(data_embs[:train_len[0]].shape)
    print(data_embs[train_len[1]:].shape)
    test_data = np.concatenate([data_embs[:train_len[0]], data_embs[train_len[1]:]])
    intrusion_detection = OCSVM()
    t0=time()
    print('model train .....')
    intrusion_detection.train(train_data)
    print('训练时间：', time()-t0)
    intrusion_detection.predict(args, test_data, flow_with_preds)
